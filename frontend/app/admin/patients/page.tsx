...
function AddPatientDialog({ onAdd, staff }: AddPatientDialogProps) {
...
export default function PatientsPage() {
  ...
  const loadData = () => {
    ...
    Promise.all([
      ...
      .catch((err) => {
        console.error("Lỗi khi tải dữ liệu SQL Server:", err);
        setLoading(false);
      });
  };
  ...
  const handleAddPatient = (newPatient: Patient) => {
    fetch(`${API_URL}/patients`, {
      ...
      .catch((err) => {
        console.error("Lỗi thêm bệnh nhân vào SQL Server:", err);
      });
  };
  ...
  const handleDeletePatient = (id: string) => {
    fetch(`${API_URL}/patients/${id}`, {
      ...
      .catch((err) => {
        console.error("Lỗi xóa bệnh nhân khỏi SQL Server:", err);
      });
  };
