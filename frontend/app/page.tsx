              ...
              >
                ...
                <div className="flex flex-col items-center text-center space-y-6 w-full mt-6">
                  ...
                  <div className="space-y-3">
                    ...
                    {person.licenses && person.licenses.length > 0 && (
                      <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-50 border border-rose-200 text-rose-700 text-[9px] font-black uppercase tracking-wider mt-1">
                        <Award className="w-3 h-3 text-rose-600 shrink-0" />
                        <span>CCHN: {person.licenses[0].licenseNumber}</span>
                      </div>
                    )}
                  </div>
                </div>
                        ...
                        <div className="space-y-3.5 text-xs font-semibold text-slate-600">
                          {person.licenses && person.licenses.length > 0 && (
                            <div className="flex items-center gap-3">
                              ...
                              <span className="font-bold text-rose-900 text-[11px] truncate">
                                CCHN: {person.licenses[0].licenseNumber}
                              </span>
                            </div>
                          )}
                          <div className="flex items-center gap-3">
    ...
    fetch(`${API_URL}/visits${queryParam}`)
      ...
      .then((data) => {
        const formatted = Array.isArray(data)
          ...
          : [];
        setMyBookings([...formatted].sort((a, b) => String(b.id).localeCompare(String(a.id))));
      })
      .catch((err) => {
        ...
        if (stored) {
          ...
          if (Array.isArray(parsed)) {
            setMyBookings([...parsed].sort((a: any, b: any) => String(b.id).localeCompare(String(a.id))));
          }
        }
      });
                  ...
                  {profileDropdownOpen && (
                    ...
                    >
                      ...
                      <div className="p-2">
                        ...
                        <button
                          onClick={() => { window.location.href = "/lich-hen"; setProfileDropdownOpen(false); }}
                          suppressHydrationWarning
                          className="w-full flex items-center gap-3.5 px-4 py-3 rounded-2xl hover:bg-blue-50 transition-all text-left group cursor-pointer"
                        >
